...
import { CategoryViewTracker } from '@/components/storefront/CategoryViewTracker';
...
// ============================================
// Category Page - Optimized with Cache & Translations
// ============================================
...
function Pagination({ currentPage, totalPages, storeSlug, handle }: PaginationProps) {
  ...
  return (
    <div className="flex justify-center items-center gap-2 mt-8">
      {currentPage > 1 && (
        <Link
          href={`/shops/${storeSlug}/categories/${handle}?page=${currentPage - 1}`}
      ...
      {pages.map((page) => (
        <Link
          key={page}
          href={`/shops/${storeSlug}/categories/${handle}?page=${page}`}
...
function EmptyCategory({ storeSlug, viewAllText }: EmptyCategoryProps) {
  return (
    <div className="text-center py-12">
      <p className="text-gray-500 text-lg mb-4">אין מוצרים בקטגוריה זו כרגע</p>
      <Link
        href={`/shops/${storeSlug}/categories/all`}
