...
// GET /api/payments/integrations/[id] - Get single payment integration
...
export async function PUT(
  ...
  try {
    ...
    const existing = await queryOne<{ id: number }>(
      'SELECT id FROM store_payment_integrations WHERE id = $1 AND store_id = $2',
      [id, user.store_id]
    );
    ...
    if (terminal_number !== undefined) {
      updates.push(`terminal_number = $${paramIndex++}`);
      values.push(terminal_number);
    }
    ...
    if (password !== undefined) {
      ...
      values.push(password); // Should be encrypted
    }
    if (api_key !== undefined) {
      updates.push(`api_key_encrypted = $${paramIndex++}`);
      values.push(api_key); // Should be encrypted
    }
...
export async function DELETE(
  ...
  try {
    ...
    const result = await queryOne<{ id: number }>(
      'DELETE FROM store_payment_integrations WHERE id = $1 AND store_id = $2 RETURNING id',
      [id, user.store_id]
    );
