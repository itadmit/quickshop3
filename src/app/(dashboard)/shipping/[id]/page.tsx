    ...
    <div className="p-6 space-y-6" dir="rtl">
      ...
      <Dialog open={showRateDialog} onOpenChange={setShowRateDialog}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {/* תצוגה מקדימה בצ'ק אאוט */}
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 className="text-sm font-semibold text-gray-900 mb-3">תצוגה מקדימה בצ'ק אאוט</h3>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-gray-900 mb-1">
                    {rateForm.name || 'שם התעריף'}
                  </div>
                  {(rateForm.delivery_days_min || rateForm.delivery_days_max) && (
                    <div className="text-sm text-gray-600 mb-1 flex items-center gap-1">
                      <HiClock className="w-4 h-4 text-gray-400" />
                      {rateForm.delivery_days_min && rateForm.delivery_days_max ? (
                        <>משלוח עד {rateForm.delivery_days_max} ימי עסקים</>
                      ) : rateForm.delivery_days_min ? (
                        <>משלוח עד {rateForm.delivery_days_min} ימי עסקים</>
                      ) : (
                        <>משלוח עד {rateForm.delivery_days_max} ימי עסקים</>
                      )}
                    </div>
                  ...
                  {rateForm.free_shipping_threshold && !rateForm.is_pickup && (
                    <div className="text-xs text-green-600 mt-1 flex items-center gap-1">
                      <HiCheckCircle className="w-3 h-3" />
                      משלוח חינם מעל ₪{parseFloat(rateForm.free_shipping_threshold).toLocaleString('he-IL')}
                    </div>
                  )}
                </div>
                <div className="text-right flex-shrink-0">
                  {rateForm.is_pickup ? (
                    <span className="text-sm font-medium text-gray-900">איסוף עצמי</span>
                  ) : rateForm.free_shipping_threshold ? (
                    <span className="text-sm font-medium text-green-600">חינם</span>
                  ) : rateForm.price ? (
                    <span className="text-sm font-medium text-gray-900">
                      ₪{parseFloat(rateForm.price).toLocaleString('he-IL')}
                    </span>
                  ) : (
